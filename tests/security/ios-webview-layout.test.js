...
test("training fields constrain their grid tracks and the appearance preference has a WebView-safe native control", () => {
  const sessionBuilder = read("src/components/training/SessionBuilder.jsx");
  const adaptiveSelect = read("src/components/ui/adaptive-select.jsx");
  ...
  assert.match(adaptiveSelect, /nativeOnMobile/);
  assert.match(adaptiveSelect, /<select/);
  assert.match(more, /nativeOnMobile/);
});
