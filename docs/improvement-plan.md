...
#### Replace framer-motion with CSS transitions + global prefers-reduced-motion
...
- **Approach:** framer-motion's only use is one AnimatePresence opacity+translateX transition on non-tab routes (AppLayout.jsx:3,58-70); it lands ~39KB gz / ~117KB parse in the shell chunk loaded on every authenticated session. Replace with a GPU-composited CSS keyframe keyed on route, then npm rm framer-motion. Combine with the currently-absent reduced-motion story: add a global @media (prefers-reduced-motion: reduce) block in index.css covering the new transition plus MacroBar (:16), ProgressRing (:23) and PullToRefresh. Replicating AnimatePresence's exit animation in pure CSS is the fiddly part (React unmounts immediately), hence M not S.
