...
- **`main` is a protected branch, and that blocks the Base44 Builder.** The GitHub API reports `protected: true` on `main` and `false` on every other branch. PR merges with green checks pass; direct pushes do not — so the Builder, which pushes straight to `main`, is refused. It surfaces in Base44 as *"A branch rule or a missing permission on this repository blocked the push. Your changes are safe – update the rule on GitHub to sync them."* The changes stay in Base44 until the push succeeds; fix the rule, then re-sync.

  The evidence for the diagnosis: the Builder's last direct push to `main` was `fbc3cdb` (2026-09-12), and everything after it (#127 onward) is a PR squash merge.

  To unblock, either add `base44-builder[bot]` to the rule's bypass list, or point the Builder at a non-default branch in the Base44 dashboard so its work arrives as a PR. The second preserves the gate and is the better destination, but `base44/config.jsonc` carries only a name and build commands with no branch setting, so whether the dashboard exposes that is **unverified** — check before assuming. Neither option is configurable from this repository, and the branch-protection rules cannot be read or changed through the GitHub MCP toolset either: `list_branches` exposes only the boolean.

  The repository side of the second route is already built: `.github/workflows/base44-builder-pr.yml` opens a pull request into `main` whenever anything is pushed to `base44-builder`, and that branch is listed in `ci.yml`'s `push.branches`. **Both halves are required.** A pull request opened with `GITHUB_TOKEN` does not start a workflow run, so without the push trigger the PR would carry no checks — a gate that gates nothing. The consequence to keep in mind is that a push-triggered run tests the branch tip, not the merge with `main`, so it does not prove integration against a `main` that has moved; swapping `GITHUB_TOKEN` for a PAT or App token is the upgrade that restores true merge testing. Nothing happens until the Builder is actually pointed at that branch.

- **Why a bypass is less dangerous than it was.** Before, ungated Builder pushes meant `main` sat red for 18 consecutive commits (`c438328`..`fbc3cdb`) across four days, visible only in the Actions tab. `ci.yml` now opens a tracked issue titled "CI is failing on main" on a red push and closes it when `main` goes green, so a red `main` announces itself. Still a smoke alarm rather than a lock — but the silent-failure mode that made ungated pushes costly is the part that has been fixed.

  An earlier version of this doc asserted flatly that "pushes to `main` are not gated." That was inferred from the 18-commit red streak without ever querying the protection flag, and it is false. Verify current state with `list_branches` rather than trusting this paragraph.
